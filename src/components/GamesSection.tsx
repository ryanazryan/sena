...
const StudentGamesView = ({ user }: StudentGamesViewProps) => {
  ...
  const highestApprovedScores = useMemo(() => {
    ...
    const approvedSubmissions = submissions.filter(
      sub => sub.status === 'graded' || sub.status === 'approved'
    );
                ...
                <CardContent className={`p-4 flex items-center justify-between ${isLocked ? 'opacity-50' : ''}`}>
                  ...
                  >
                    {isLocked ? (
                      ...
                      <><PlayCircle className="w-4 h-4 mr-2" /> {`Mulai Level ${index + 1}`}</>
                    )}
                  </Button>
                </CardContent>
              </Card>
              ...
              <div>
                ...
                <select id="game-select" className="w-full p-2 border rounded-lg bg-input mt-1" value={game} onChange={(e) => setGame(e.target.value)}>
                  {availableGames.map((g, index) => (
                    <option key={g.id} value={g.name}>{`Level ${index + 1}: ${g.name}`}</option>
                  ))}
                </select>
              </div>
              <div>
              ...
              <div className="mt-1">
                {previewUrl ? (
                  ...
                  <label htmlFor="file-upload" className="border-2 border-dashed border-border rounded-lg p-6 text-center hover:border-primary transition-colors cursor-pointer block">
                    ...
                    <p className="text-sm text-muted-foreground mb-2">Klik untuk upload screenshot</p>
          ...
          {reviewingSubmission && (
            <>
              <DialogHeader className="p-6 flex-shrink-0 border-b">
                <DialogTitle>Game{reviewingSubmission.game}</DialogTitle>
                <DialogDescription className="flex flex-wrap items-center gap-x-4 gap-y-2 pt-2">
                  <span className="font-medium">
                    ...
                    <span className="text-sm text-muted-foreground ml-2">
                      ({userInfoMap.get(reviewingSubmission.userId)?.email})
                    </span>
                  </span>
              ...
              <div className="overflow-y-auto">
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <div className="border rounded-lg overflow-hidden aspect-video w-full max-w-xl mx-auto">
                      <ImageWithFallback
                    ...
                    <div className="p-3 bg-muted rounded-lg border min-h-[50px]">
                      <p className="text-sm">{reviewingSubmission.note || "Siswa tidak memberikan catatan."}</p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <h4 className="font-medium">Verifikasi & Feedback</h4>
                    <p className="text-sm text-muted-foreground">Verifikasi skor dan berikan feedback.</p>
                    ...
                    {reviewingSubmission.status === 'graded' && reviewingSubmission.feedback && (
                      <div className="space-y-2">
                        ...
                        <div className="p-3 bg-muted rounded-lg border overflow-y-auto max-h-[200px]">
              ...
              <DialogFooter className="p-6 border-t flex-shrink-0">
                {approvalError && (
                  <div className="w-full text-sm text-destructive flex items-center gap-2 mb-2">
                    <AlertCircle className="w-4 h-4" /> {approvalError}
                  </div>
                )}
                <Button variant="destructive" onClick={() => { /* handleReject logic */ }}>
                  <X className="w-4 h-4 mr-2" /> Tolak
                </Button>
                <Button onClick={() => { /* handleApprove logic */ }} className="bg-green-600 hover:bg-green-700">
                  <Check className="w-4 h-4 mr-2" />
                  {reviewingSubmission.status === 'graded' ? 'Perbarui Feedback' : 'Setujui'}
                </Button>
              </DialogFooter>
            </>
          )}
